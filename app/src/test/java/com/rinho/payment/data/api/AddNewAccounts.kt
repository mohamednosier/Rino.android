...
class AddNewAccounts {
    ...
    @Test
    fun login() {
        enqueueResponse("newaccount.json")


        val userAuth = (service.insertaccount(createInsertAccount()).getOrAwaitValue() as ApiSuccessResponse).body?.data
...
//                .addHeader("Accept-Language", Locale.getDefault().getLanguage())
    ...
    fun `fetch details and check response success returned`(){
        ...
        val response = MockResponse()
            ...
            .setBody(MockResponseFileReader("newaccount.json").content)
        mockWebServer.enqueue(response)
        val mockResponse = response.getBody()?.readUtf8()
        // Act
//        val  actualResponse = apiHelper.getEmployeeDetails().execute()
        val actualResponse = (service.insertaccount(createInsertAccount()).getOrAwaitValue() as ApiSuccessResponse).body?.data.toString()
        // Assert
